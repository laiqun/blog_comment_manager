 ...
 * 让人工确认 AI 找到的是哪个表单（与红色「定位同行网站」标记互不干扰）；
 * 「定位同行网站」由面板助手页触发（locateLink）：循环跳转到页面中包含收集目标域名
...
(function () {
  ...
  // ---------- 高亮标记：红框=定位同行网站 / 蓝框=AI 识别出的表单 ----------

  let linkIdx = 0; // 「定位同行网站」循环跳转的游标
