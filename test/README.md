...
| `storage.test.mjs` | 任务计数（含待审核）、日志上限、save 序列化（内存版 chrome.storage 桩） |
...
| `service-worker.test.mjs` | getSnapshot 时按 IndexedDB 口径刷新收集统计（已发现/已分析/队列中/命中），运行中不刷新，targetDomain 丢失时从 IDB 恢复；快照 resources 由 analysis 命中记录派生（enabled/published 标记）；资源库读 analysis 表与启停开关（内存版 chrome.* + IndexedDB 桩） |
